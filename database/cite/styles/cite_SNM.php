		...
		if (preg_match("/^(Journal Article|Magazine Article|Newspaper Article)$/", $row['type']))
			{
				...
				if (!empty($row['publication']))      // publication (= journal) name
					$record .= " in: " . $markupPatternsArray["italic-prefix"] . $row['publication'] . $markupPatternsArray["italic-suffix"];
				...
				if (!empty($row['pages']))      // pages
				{
					...
					$record .= formatPageInfo($row['pages'], $markupPatternsArray["endash"], "", ""); // function 'formatPageInfo()' is defined in 'cite.inc.php' (NOTE: from the examples in the above mentioned guide it's unclear whether "p." should be followed by a space or not)
				}
